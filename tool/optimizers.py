...
def init_optimizer(model,
    ...
    if staged_lr:
        ...
        param_groups = list(model.resnet50.parameters())
        param_groups += list(model.img_rnn1.parameters())
        param_groups += list(model.img_rnn2.parameters())
        #param_groups += list(model.img_rnn3.parameters())
        param_groups += list(model.AttentionLayer.parameters())
        param_groups += list(model.dropout.parameters())
        param_groups += list(model.final_mlp.parameters())
        #param_groups += list(model.fc.parameters())
        #param_groups += list(model.GCN1.parameters())
        #param_groups += list(model.GCN2.parameters())
        #param_groups += list(model.GCN3.parameters())
